...
bool SetFireWallRuleDescription(CComPtr<INetFwRule>& pRule,
                                const std::string& description)
{
    std::shared_ptr<WCHAR> bstrRuleDescription =
        ConvertStdStringToSystemString(description);
...
bool SetFireWallRuleDirection(CComPtr<INetFwRule>& pRule,
                              const NET_FW_RULE_DIRECTION flag)
{
...
{
    std::shared_ptr<WCHAR> bstrRuleAddress =
        ConvertStdStringToSystemString(address);
...
bool DeleteRule(CComPtr<INetFwRules>& rules, const std::string& rule_name)
{
    ...
    {
        std::shared_ptr<WCHAR> bstrRuleName =
            ConvertStdStringToSystemString(rule_name);
...
bool DeleteRulesByRegex(const std::string& expr)
{
    ...
    ULONG cFetched = 0;
    for (CComVariant var; pVariant->Next(1, &var, &cFetched) == S_OK;
         var.Clear())
    {
