...
namespace firewall_controller {
	...
	bool SetFireWallRuleDescription(CComPtr<INetFwRule>& pRule, const std::string& description = "Auto Create Rule.");
	bool SetFireWallRuleDirection(CComPtr<INetFwRule>& pRule, const NET_FW_RULE_DIRECTION flag = NET_FW_RULE_DIR_IN);
	bool SetFireWallRuleProtocol(CComPtr<INetFwRule>& pRule, const LONG flag = NET_FW_IP_PROTOCOL_TCP);
	...
	bool SetFireWallRuleAction(CComPtr<INetFwRule>& pRule, const NET_FW_ACTION flag = NET_FW_ACTION_ALLOW);
